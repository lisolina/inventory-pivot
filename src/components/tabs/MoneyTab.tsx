...
export const MoneyTab = () => {
  ...
  const fetchAll = async () => {
    const [invRes, expRes, cashRes] = await Promise.all([
      ...
      supabase.from("cash_entries").select("*").order("date", { ascending: false }).limit(50),
    ]);
    if (invRes.data) setInvoices(invRes.data);
    if (expRes.data) setExpenses(expRes.data);
    if (cashRes.data) {
      setCashEntries(cashRes.data);
      if (cashRes.data.length > 0 && cashRes.data[0].balance_after !== null) {
        setCashBalance(Number(cashRes.data[0].balance_after));
      }
    }
  };
      ...
      <Card className="border-accent/30">
        ...
        <CardContent>
          <div className="text-4xl font-bold">
            {cashBalance !== null ? `$${cashBalance.toLocaleString()}` : "— Not set yet"}
          </div>
                  ...
                  <TableBody>
                    {expenses.map((exp) => (
                      <TableRow key={exp.id}>
                        ...
                        <TableCell>
                          <Badge variant={exp.status === "paid" ? "default" : "outline"} className={exp.status === "paid" ? "bg-success text-success-foreground" : ""}>
                            {exp.status}
                          </Badge>
                        </TableCell>
                      </TableRow>
          ...
          <Card>
            <CardContent className="pt-6">
              {cashEntries.length === 0 ? (
                <p className="text-center text-muted-foreground py-8">No cash entries yet.</p>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
